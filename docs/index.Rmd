...
date: "2024-11-01"
---
# Pregnancy termination trend over 8 year period in light of contraceptives dispensation trends
...
health_board_population <- left_join(health_board_population, health_boards, by = "hb_name") %>% 
...
```{r Figure 1, fig.cap='Fig 1. Gross prescription of contraceptives between 2016 and 2023'}
...
### Contraceptive trends in NHS Scotland health boards



```{r Figure 3,  fig.cap=c('Fig. 3: Long-term contraceptive item prescription across NHS Scotland boards', 'Fig. 4: Short-term contraceptive item prescription across NHS Scotland boards')}

plot_across_boards <- function(starting_data){
  plot_prescription <- starting_data %>% 
  ungroup() %>%
  mutate(hb_name1 = hb_name) %>% 
  select(-hb_name) 
  
  ggplot() +
  geom_line(data=plot_prescription , aes(group=hb_name1, y = contracep_items_per_1000_women, x = year), color="grey", size=0.5, alpha=0.5) +
  geom_line(data = starting_data, aes(group = hb_name, color=hb_name, y = contracep_items_per_1000_women, x = year), size=1.2 )+
  theme_classic() +
  theme(plot.title = element_text(size = 13, 
                                  hjust = 0,
                                  face = "bold", 
  ...
  labs(title = "Long-term contraceptive item prescription across NHS Scotland boards", x = "Year", y = "Number of items per 1000 women") +
  facet_wrap(~hb_name, nrow = 3, 
             labeller = label_wrap_gen(18))
}

plot_across_boards(long_term_contraception)
plot_across_boards(short_term_contraception)
