...
columns_needed_from_prescription <- c("BNFItemDescription", "NumberOfPaidItems")

contraceptives <- c("MERCILON", "GEDAREL", "FEMODETTE", "MILLINETTE", "SUNYA", "LOESTRIN",
 "MARVELON", "CIMIZT", "YASMIN", "LUCETTE", "FEMODENE", "KATYA", "LEVEST",
 "MICROGYNON", "OVRANETTE", "RIGEVIDON", "CILEST", "LIZINNA", "BREVINOR",
 "OVYSMEN", "NORIMIN", "NORINYL", "CERAZETTE", "CERELLE", "ZELLETA",
 "NORIDAY", "NORGESTON")
...
read_and_clean <- function(url) {
  ...
  data <- vroom(url, col_select = any_of(c(columns_needed_from_prescription, "HBT", "HBT2014")), skip_empty_rows = TRUE)
   ...
   # contraceptives names separated with | operator to later pass it to filtering to look for either of the drug names in the drug names column
  drug_name_with_OR <- paste(contraceptives, collapse = "|")
 
  #data filtered for contraceptives only
  data_filtered <- data %>%
  filter(str_detect(BNFItemDescription, drug_name_with_OR)) %>% 
    clean_names()
...
```{r Sum contraceptives per health board}
#sum the number of items paid for by health board
summarise_and_add_data <- function(url) {
  name <- names(prescription_data_urls)[which(prescription_data_urls == url)] 
  data <- get(name) 
  data <- data %>% 
    group_by(hb) %>% 
    summarise(sum_of_contraceptives_paid = sum(number_of_paid_items, na.rm = TRUE)) %>% 
...
lapply(prescription_data_urls, summarise_data)
```


```{r Combine all datasets into one}

full_prescirption_dataset <- bind_rows(mget(paste(names(prescription_data_urls), "sum", sep = "_")))

full_prescirption_dataset_grouped <- full_prescirption_dataset %>% 
  group_by(year) %>% 
  summarise(number_of_contraceptives_per_year = sum(sum_of_contraceptives_paid))
...
```{r}
plot_prescription <- full_prescirption_dataset_grouped %>% 
  ggplot(aes(y = number_of_contraceptives_per_year, x = year)) +
  geom_line() 
plot_prescription

plot_abortion <- abortion_data %>% 
  filter(age_group == "All Ages", as.numeric(yearof_termination) %in% c(2018,2019,2020,2021,2022,2023)) %>% 
  group_by(yearof_termination) %>% 
  summarise(number_of_terminations_per_year = sum(numberof_terminations, na.rm = TRUE)) %>% 
  ggplot(aes(y = number_of_terminations_per_year, x = yearof_termination)) +
  geom_line() 
plot_abortion
```
