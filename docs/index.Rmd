...
#URLs for prescription data
```{r}
#prescription data between 2018 and 2023
...
```{r}
data <- vroom(prescription_data_urls[["dec_2023"]], col_select = columns_needed_from_prescription, skip_empty_rows = TRUE)
...
```{r}
#iterate eahc object in the named list of prescription url
for (name in names(prescription_data_urls))
{
  data <- vroom(prescription_data_urls[[name]], col_select = any_of(c(columns_needed_from_prescription, "HBT", "HBT2014")), skip_empty_rows = TRUE)
  ...
  #use assign function to create a value with the name of the url name and assign read url to that variable
  assign(name, data_filtered) 
}
