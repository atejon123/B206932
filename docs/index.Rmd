...
health_board_population <- read_csv("https://s3-eu-west-1.amazonaws.com/statistics.digitalresources.jisc.ac.uk/dkan/files/2022/NRS/UV103/census_2022_UV103_age_Health_Board_Area_2019.csv", skip = 10) %>% 
  clean_names() %>% 
  rename(hb_name = "health_board_area_2019") %>% 
  filter(age == "All people") %>% 
  select(hb_name, count)
```
...
```{r Joining health boards with data}
...
plot_abortion <- abortion_data %>% 
  ...
  ggplot(aes(y = numberof_terminations, x = yearof_termination, colour = hb_name)) +
  geom_line() 
plot_abortion
```
