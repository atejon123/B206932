...
Nevertheless, female contraceptives remain the most reliable way to prevent unwanted pregnancy [3]. The changes in birth control usage probe the question of how this correlates with further measures to terminate unwanted pregnancy. This report will explore female contraceptive trends in Scotland between 2018 and 2023 and look at how this trend correlated with pregnancy termination statistics.
...
To ensure comparable figures between different boards the number of pregnancy termination events was normalised per 1000 women in the health board per [2022 Cencus data](https://www.scotlandscensus.gov.uk/webapi/jsf/tableView/tableView.xhtml) (Scotland's Census 2022 - National Records of Scotland
...
# combining health board names and populations removing NHS from Health board name for readability in plots and adding Islands population to the full dataframe
...
```{r Data reading and cleaning, results = "hide"}
...
boards_to_sum <- c("Orkney", "Shetland", "Western Isles")
...
Figure 1 shows values normalised to 2016 for long-term and short-term contraceptives. There seems to be a nationwide decline in birth control usage. It can be seen that short-term contraception has been on a consistent decline over the past 8 years. Covid doesn’t seem to have had a significant impact on these prescriptions. However, Covid hindered access to long-term contraceptives. That is likely because the application of intravaginal devices and implants requires medical professionals and during Covid clinics’ had to focus their resources on emergency medical care. 

Because of strong Covid's effect on long-term contraceptives it is difficult to see the overall trend. In years before Covid there was a downward trend from 2016 to 2018 but contraception usage spiked in 2019 for both types of contraceptives. It is unclear whether long-term contraceptives use would have continued to grow if Covid didn't have en effect, however seeing that similar spike happened for short-term contraceptives in 2019 it seems unlikely. Therefore, it doesn't look like decline in short-term contraceptives usage can be explained by the shift towards long-term options since trends for both types are very similar apart form Covid period. This overall decline in contraceptives use might be explained by growing worries about using hormonal contraceptives [1]. 


```{r Figure 1, fig.cap='Fig. 1: Normalised prescription of long-term and short-term contraceptives'}
full_prescirption_dataset %>%
  ...
  mutate(total_2016 = total[year == 2016], total_normalised = total / total_2016) %>% 
        ...
        geom_point(size = 3) +
        geom_vline(xintercept = c(2020,2022), color="grey", size=.5, linetype = "dashed") +
        theme_classic() +
        ...
        scale_color_manual(values = c("#6D91F2", "#6DF2A6"))
```
...
A negative trend in contraceptive usage can be seen across all health boards (Fig. 2). Most of the health boards have very similar use and trends of contraception, except for Lothian and Dumfries and Galloway. Lothian and Dumfries and Galloway have different contraceptive rates to other boards. Lothian has a significantly lower dispensation of contraceptives, while Dumfries and Galloway has significantly higher. Further analysis is needed to better understand the reasons behind these differences. On average, `r contraceptives_mean_2016` items of contraceptives were dispensed per 1000 in each Health Board in 2016. This number dropped to `r contraceptives_mean_2023` in 2023. While all health boards showcase a decline in contraceptive use, Grampian, Islands, and Lanarkshire had the sharpest decrease from 2016 to 2023.

Table 1 shows a change from 2016 to 2023 in dispensed contraceptives and number of pregnancies terminated for each health board in Scotland. Values higher than the mean are highlighted in red. It can be seen that all health boards experienced an increase in the number of terminated pregnancies with Lanarkshire having almost double the average of an increase.

```{r, fig.cap='Fig. 2: Contraceptives prescription in each NHS Scotland health board'}
...
```{r Changes in aboriton and contraception usage from 2016 to 2023, paged.print=TRUE, fig.cap='Table 1. Pregnancy terminations and contraceptive usage change in each health board from 2016 to 2023. Values higher than the mean are highlighted in red.' }
...
```{r,  fig.cap="Fig 3. Correlation between aboirtion and ocntraception figures in each health board"}
  combined_data %>%  
