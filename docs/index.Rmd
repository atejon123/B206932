...
3. Is there a correlation between contraceptive trend and pregnancy termination trend.
...
To ensure comparable figures between different boards the number of pregnancy termination events was normalised per 1000 of women in the health board in accordance to (2022 Cencus data)[https://www.scotlandscensus.gov.uk/webapi/jsf/tableView/tableView.xhtml]. While the total population might change over the years we are expecting that he change wont be significant to impact the overall trends. Only women of biological reproductive age were included as the members of this group are the most likely to get pregnant. The chosen reproductive age span was from 13 (average age of first menstruation) to 50 (average age of natural menopause) [4].
...
# combining health board names and populations removing NHS from Health board name for readibility in plots and adding Islands population to the full dataframe
...
# abortion data joined to health board names and populations, number of abortion per 1000 women added
...
# apply the outlined function to all data urls
...
add_year <- function(url) {
  ...
  #get the name of the url form the named list and turn that name into object
  name <- names(prescription_data_urls)[which(prescription_data_urls == url)] 
  
  # get the selected object from the environment
  data <- get(name) 
  
  # add a column with year by extracting the number from the name
  data <- data %>% 
    add_column(year = as.numeric(str_extract_all(name, "\\d+")))
  
  #create a new object for each modified dataframe
  object_name <- paste(name, "sum", sep = "_")
  assign(object_name, data, envir = .GlobalEnv) 
}

# apply the above function to all dataframes read from urls
...
long_term_contraception <- full_prescirption_dataset %>% 
  ...
  group_by(year) %>% 
  summarise(sum = sum(number_of_paid_items, na.rm = TRUE)/29109) 

short_term_contraception <- full_prescirption_dataset %>% 
  filter(!str_detect(bnf_item_code, "0703022P|0703023|210400002")) %>% 
  group_by(year) %>% 
  summarise(sum = sum(number_of_paid_items, na.rm = TRUE)/865093)


```

```{r Contraceptive and abortion data per health board, include=FALSE}
...
# total contraceptives for all health boards representing Islands health board summed together for each year 
...
# abortion data filtered for years of interest
...
It has been shown that over the past 8 years there has been a decline in combined oral contraceptives being prescribed while the progesterone only pill prescriptions increased [5]. While it shows that women preference might be shifting away from estrogen containing products it doesn't tell much about the overall trend in contraceptive usage. 

Figure 1 shows values normalised to 2016 for long-term and short-term contraceptives. There seems to be a nation wide decline in birth control usage. It can be seen that short-term contraception is on a consistent decline over the past 8 years. Covid doesn't seem to have had significant impact on these prescriptions. However, Cocid hindered access to long-term contraceptives. That is likely because application of intravaginal devices and implants require medical professionals and during Covid clinics' had to focus their resources on emergency medical care. Overall, the rise in contraceptive use in 2019 seems to be influenced by both types of contraceptives.


```{r Figure 1, echo = FALSE, fig.cap='Fig. 1: Normalised prescription of long-term and short-term contraceptives'}
...
Negative trend in contraceptive usage can be seen across all health boards (Fig. 2). Most of the health boards have very similar use and trend of contraception, except for Lothian and Dumfries and Galloway. While Lothian and Dumfries and Galloway have similar overall trend to other boards, Lothian has significantly lower dispensation of contraceptives while Dumfries and Galloway has significantly higher. Further analysis is needed to better understand the reasons behind these differences. On average, 736.05 items of contraceptives were dispensed per 1000 in each Health Board in 2016. This number dropped to 626.76 in 2023. While all health boards showcase a decline in contraceptive use, Grampian, Islands, and Lanarkshire had the sharpest decrease from 2016 to 2023. 

Table 1 shows change from 2016 to 2023 in dispensed contraceptives and number of pregnancies terminated for each health board in Scotland. Values higher than the mean are highlighted in red. It can be seen that all health boards experienced increase in number of terminated pregnancies with Lanarkshire having a more than double the averse of an increase.

```{r, echo = FALSE, fig.cap='Fig. 2: Contraceptives prescription in each NHS Scotland health board'}
...
ggplot() +
  geom_line(data=plot_prescription , aes(group=hb_name1, y = contraceptives_per_1000_women, x = year), color="grey", size=0.5, alpha=0.5) +
  geom_line(data = combined_data, aes(group = hb_name, color=hb_name, y = contraceptives_per_1000_women, x = year), size=1.2 ) +
...
# mean of contraceptive items in 2016
contraceptives_mean_2016 <- combined_data %>% 
  filter(year == 2016) %>% 
  summarise(mean =mean(contraceptives_per_1000_women)) %>% 
  pull(mean)

# mean of contraceptive items in 2023
contraceptives_mean_2023 <- combined_data %>% 
  filter(year == 2023) %>% 
  summarise(mean =mean(contraceptives_per_1000_women)) %>% 
  pull(mean)
```
...
```{r Changes in aboriton and contraception usage from 2016 to 2023, echo=FALSE}
...
combined_data_2016_2023_diff %>% 
  gt() %>% 
  tab_header(title = md("**Table 1.** Change in Abortion and Contraceptives Prescription Rate Across Scotland Health Boards from 2016 to 2023")) %>% 
  cols_label(diff_abor = md("**Abortion Rate Change**"), diff_contr = md("**Contraceptives Rate Change**"), hb_name = md("**Health Board**"))%>% 
  cols_align(align = "center",
             columns = c(diff_abor, diff_contr)) %>% 
  tab_spanner(label = "Calculated per 1000 of women", columns = c("diff_abor", "diff_contr")) %>% 
  grand_summary_rows(columns = c(diff_abor, diff_contr),
                     fns = list("Overall Mean Change" = ~mean(., na.rm = TRUE)),
  ...
  tab_style(
    locations = cells_body(
      columns = `diff_contr`,
      rows = `diff_contr` < (-109.29)
    ),
    style = list(cell_text(color = 'red')))

```

### Contraceptive and pregnancy termination correlation across NHS Scotland Health Boards

With contraceptives dispensation on a decline it begs a question of how this might correlate with pregnancy termination data. Each colored point in Figure 3 represents a data value from a single year, plotted with abortion rates on the y axis and contraceptive use on the x axis. It is evident that there is a relationship between abortion and contraceptives use. Abortion dicreses in all heath boards when contraceptives prescriptions increase. However, several health boards stand out with slightly different rates from other boards. Dumfries and Galloway has been mentioned before as the board with the highest contraceptive consumption. Interestingly, abortion rate in this regions doesn't seem to deviate from the values of the cluster of other boards. On the other hand the Islands have much lower contraceptives use compared to Dumfries and Galloway but have around half of the abortions. Lothian, with the lowes usage of contraceptives doesn't display much higher abortion rates. Therefore, abortion rate and contraceptive usage correlates within health boards but such consensus is not maintained across health boards.



```{r, echo=FALSE, fig.cap="Fig 3. Correlation between aboirtion and ocntraception figures in each health board"}
...
To better understand the changes over the 8 year period discussed in this it is crucial to examine the reasons these changes occur. Decline in contraception use might be a direct outcome of social media portrayal of hormonal contraceptives. However, it is not fully examined what other reasons might encourage people to stop using contraception. This report showcases the negative relationship between contraception use and termination of pregnancy occurrence, however, the outlined relationship is not necessarily fatalistically significant. This needs to be explored further to draw more solid conclusions. The relationship between contraceptive use and number of pregnancy termination procedures might be indirect and influenced by a measure now reflected in a data. Therefore, further analysis is important to understand these trends.  

## References

1. Guen ML, Schantz C, Régnier-Loilier A, Rochebrochard E de L. Reasons for rejecting hormonal contraception in Western countries: A systematic review. Social Science & Medicine. 2021;284:114247. 

2. Schneider-Kamp A, Takhar J. Interrogating the pill: Rising distrust and the reshaping of health risk perceptions in the social media age. Social Science & Medicine. 2023;331:116081. 

3. Diana Mansour PI, Gemzell-Danielsson K. Efficacy of contraceptive methods: A review of the literature. The European Journal of Contraception & Reproductive Health Care. Taylor & Francis; 2010;15(1):4–16. PMID: 20136566

4. Nabhan AF, Mburu G, Elshafeey F, Magdi R, Kamel M, Elshebiny M, Abuelnaga YG, Ghonim M, Abdelhamid MH, Ghonim M, Eid P, Morsy A, Nasser M, Abdelwahab N, Elhayatmy F, Hussein AA, Elgabaly N, Sawires E, Tarkhan Y, Doas Y, Farrag N, Amir A, Gobran MF, Maged M, Abdulhady M, Sherif Y, Dyab M, Kiarie J. Women’s reproductive span: a systematic scoping review. Human Reproduction Open. 2022 Feb;2022(2):hoac005. 

5. Johnson-Hall E. Oral contraception prescribing trends in Scottish general practices between 2016 and 2023. BMJ Sexual & Reproductive Health [Internet]. British Medical Journal Publishing Group; 2024; Available from: https://srh.bmj.com/content/early/2024/05/24/bmjsrh-2024-202387
