...
We are assuming that there is a direct link between contraceptive dispensation and usage; therefore, in some parts of the report, the dispensation data is referred to as usage data.
...
Figure 1 shows values normalised to 2016 for long-term and short-term contraceptives. There seems to be a nationwide decline in birth control usage. It can be seen that short-term contraception has been on a consistent decline over the past 8 years. Covid doesn’t seem to have had a significant impact on these prescriptions. However, Cocid hindered access to long-term contraceptives. That is likely because the application of intravaginal devices and implants requires medical professionals and during Covid clinics’ had to focus their resources on emergency medical care. Overall, the rise in contraceptive use in 2019 seems to be influenced by both types of contraceptives.
...
print(ggplot() +
        geom_line(data = long_term_contraception, aes(y = sum, x = year, color = "Long-term"), linewidth = 2) +
        geom_line(data = short_term_contraception, aes(y = sum, x = year, color = "Short-term"), linewidth=2) +
        ...
        labs(title = "Normalised prescription of long-term and short-term contraceptives", 
             x = "Year", 
             y = "Normalised value of items dispensed per year",
             colour = "Legend") +
        annotate(geom="text", x=2021, y=1, 
                 label="COVID period", colour = "#71797E")  +
        scale_color_manual(values = c("Long-term" = "#6D91F2", "Short-term" = "#6DF2A6"))
)
...
A negative trend in contraceptive usage can be seen across all health boards (Fig. 2). Most of the health boards have very similar use and trends of contraception, except for Lothian and Dumfries and Galloway. Lothian and Dumfries and Galloway have different contraceptive rates to other boards. Lothian has a significantly lower dispensation of contraceptives, while Dumfries and Galloway has significantly higher. Further analysis is needed to better understand the reasons behind these differences. On average, 736.05 items of contraceptives were dispensed per 1000 in each Health Board in 2016. This number dropped to 626.76 in 2023. While all health boards showcase a decline in contraceptive use, Grampian, Islands, and Lanarkshire had the sharpest decrease from 2016 to 2023.

Table 1 shows a change from 2016 to 2023 in dispensed contraceptives and number of pregnancies terminated for each health board in Scotland. Values higher than the mean are highlighted in red. It can be seen that all health boards experienced an increase in the number of terminated pregnancies with Lanarkshire having more than double the averse of an increase.
...
```{r Changes in aboriton and contraception usage from 2016 to 2023, include=FALSE}
...
```{r Changes in aboriton and contraception usage from 2016 to 2023, echo=FALSE, paged.print=TRUE}
